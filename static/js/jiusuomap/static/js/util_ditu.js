/**
 * Created by huadong on 2016/11/23.
 */
$(function(){

setTimeout(function(){
    $('.ditu_index_left .name').click(function(){
        $(this).next('ul').toggle();
    });
    $('.ditu_index_right .list_first .list_first_img').click(function(e){
        e.stopPropagation();
        $(this).parents().find('.pop').finish().toggle();
    });
    $('.ditu_index_right .list_inner').click(function(e){
        e.stopPropagation();
        $(this).siblings().finish().toggle('slow');
        $(this).parent().siblings().find('.triangle,.set_pop').hide();
    });
    var $pop=$('.ditu_index_right .pop .set');
    $pop.parent().width($pop.outerWidth(true)*$pop.size());
    $pop.hover(function(){
            $(this).find('.img_right1').hide().siblings().toggle();
        },
        function(){
            $(this).find('.img_right2').hide().siblings().toggle();

        }).click(function(){

    });
    $('.ditu_index_right .word').hover(function(e){
            e.stopPropagation();
    ...
    // $('.scale_control .back').click(function(){
    //     $(this).parents('.list').toggleClass('list_hover');
    //     $('.set_pop_scale').toggle();
    // });
    $('.set_pop_right_word .close').click(function(){
        $(this).parents('.set_pop_right').toggle();
    });
    $('.set_pop_right_pop .close').click(function(){
        $(this).parents('.set_pop_right_pop').toggle();
    });
    $('.set_pop_right_click .word:nth-child(3)').click(function(){
        $('.set_pop_right_pop').toggle();
    });
    $('.ditu_index_left .right').click(function(){
        $('.ditu_index_left').toggle();
    });
    $('.ditu_index_right .set_pop .circle').click(function(e){
        $(this).parents().toggleClass('open_radio_close');
    });
    $('.scale_control .set_pop_scale label').click(function(){
        //$(this).find('input').toggle();
    });
    $('.ditu_index_right .list_pop .word').click(function(e){
        //$('.scale_control').toggle();
    });
    $('.left_hide').click(function(e){
        $('.ditu_index_left').toggle();
    });
    $('.ditu_right_down_arrow').click(function(e){
        $('.ditu_right_down').toggle();
    ...
    /*滚动控制*/
    (function move(){
        var $p=$(".ditu_index_left");
        var _h1=$p.outerHeight(true);
        var _h2=$p.children('.top').outerHeight(true);
        var $down=$p.find(".down");
        $down.height(_h1-_h2);
        $down.panel({iWheelStep:32});
        $('.set_pop_scale .margin').panel({iWheelStep:32});
    })();
    // $(".video_img1").click(function(){
    //     $('.set_pop_right_word').toggle();
    // });
    // $(".video_img2").click(function(){
    //     $('.video_box').toggle();
    // });
    // $(".video_img3").click(function(){
    //     $('.set_pop_right_click').toggle();
    // });
    // $(".video_img4").click(function(){
    //     $('.scale_control_short').toggle();
    // });
    //rome(date1);
    //rome(date2);
    /*时间样式*/
    ...
    /*右侧信息提醒*/
    $('.ditu_index_right .list:not(".list_first")').on('mouseenter',function(){
        $(this).find('.pop_title').show();
    }).on('mouseleave',function(){
        $('.pop_title').hide();
    });
    $('.ditu_index_right .set').on('mouseenter',function(){
        $(this).find('.pop_title').show();
    }).on('mouseleave',function(){
        $('.pop_title').hide();
    });
