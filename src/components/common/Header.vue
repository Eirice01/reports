    ...
    export default {
      ...
      methods:{
        ...
        chooseTimeType(selected){
          ...
          if(selected == "minute" && this.timeVal.length <= 10){
            time.setHours(0);
            time.setMinutes(0);
          }
          ...
          this.$emit('tourChangeData',selected,time2);
          this.$emit('sourceNumChange',selected,time2);
        ...
        changeDate(type){
          ...
          //日，月选择后时间被选择后调用
          if(this.timeSelectType!='minute'){
           this.$emit('getchoseDayTime',this.timeVal);
          }
