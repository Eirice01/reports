<template>
  <div id="time-select" style="margin-bottom: 15px">
    ...
    <div class="check-list-content" v-show="true">
      <Select v-model="times">
        <Option v-for="(item,index) in timeLsit" :key="index" :value="item.id" @click.native="getChooseInfo(item)">{{item.description}}</Option>
      </Select>
    </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      //获取预制时间下拉
      getMyTimeOption(flag){
        this.$http.request({
          method:'get',
          params:{flag:flag},
          url:URL.queryTimeList,
          success:(data) => {
            if(data.code==200){
              this.timeLsit=data.data
            }
          },
          error : (data) => {
