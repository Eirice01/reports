  ...
  <div class="mapTools">
    <ul class="clearFix">
      ...
      <ButtonGroup size="large" style="margin-left: 100px;height: 38px">
        <Button class="selectedArea" @click.native="showArea('0')" :class="!flagClick ? 'default':'visited'">行政区</Button>
        <Button class="selectedArea"  @click.native="showArea('1')" :class="flagClick ? 'default':'visited'">开发区</Button>
      </ButtonGroup>
    </ul>
  </div>
</template>
  ...
  export default {
   ...
   selectAreaEvent(){
    ...
    map.on("click",function(e){
      ...
      homeThis.$emit("getAreaCode",homeThis.areaCode,homeThis.areaName);
    });
  }
    }
  }
