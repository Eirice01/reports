<template>
    <div id="area-select">
      <div class="check-list-title">
        <span class="check-list-title-left"><span class="item-warn">*</span>{{areaTitle}}：</span>
        <span class="check-list-title-right"></span>
      </div>
      <div class="check-list-content" v-if="this.$route.query.type == 'pop'">
        <Timeline>
          <TimelineItem>
          ...
          <TimelineItem>
            <Tabs name="tab-child" :value="selectName" size="small" @on-click="selectAreaTab">
              <TabPane label="开发区" name="kaifa" tab="tab-child">
                <div style="text-align: right" v-show="showSelectAll">
      ...
      <div class="check-list-content" v-else>
        <div style="text-align: right" v-show="showSelectAll">
          <Checkbox v-model="allSelectView" @on-change="selectAll(allSelectView,viewAreas)">全选</Checkbox>
          <Button  size="small" @click="resetLabel('allSelectView',viewAreas)" class="btn-primary">重置</Button>
        </div>
        <div class="view-box">
    ...
    export default {
      ...
      data() {
        return{
          ...
          day:"白天",
          areaVal:"",
          areaTitle:this.$route.query.type == "pop" ? '区域':'景区',
          areaData:["xian"],
          allSelect:false,
