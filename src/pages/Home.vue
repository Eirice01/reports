      ...
      <Row>
        ...
        <div class="headerRouter">
          <Menu mode="horizontal" :theme="theme1" class="linearBgColor" :active-name="activeName" @on-select="selectItem" ref="side_menu">
            ...
            <Submenu name="分析对比" class="f-fs4" v-if="$perms.hasPermission('key300')">
              <template slot="title">
                <span>
                  <img class="title-img" src="../assets/img/fx.png" alt="">
                </span>
                <span class="headerTitle">态势分析</span>
              </template>
              <router-link :to="{path:'/Contrast',query:{type:'pop'}}" v-if="$perms.hasPermission('key300101')">
                <MenuItem name="/Contrast" class="f-fs4">人口对比</MenuItem>
              </router-link>
              <router-link :to="{path:'/Contrast',query:{type:'tour'}}" v-if="$perms.hasPermission('key300102')">
                <MenuItem name="/Contrast2" class="f-fs4">旅游对比</MenuItem>
              </router-link>
            </Submenu>
            <MenuItem name="/Report" v-if="$perms.hasPermission('key400')">
            ...
            <MenuItem name="/Traffic" v-if="$perms.hasPermission('key600')">
              <router-link :to="{path:'/Traffic'}">
                <span>
                  <img class="title-img" src="../assets/img/bg.png" alt="">
                </span>
                <span class="headerTitle">交通</span>
              </router-link>
            </MenuItem>
            <MenuItem name="/Trajectory" v-if="$perms.hasPermission('key700')">
              <router-link :to="{path:'/Trajectory'}">
                <span>
                  <img class="title-img" src="../assets/img/bg.png" alt="">
                </span>
                <span class="headerTitle">轨迹</span>
              </router-link>
            </MenuItem>
            <Submenu name="系统管理" class="f-fs4" v-if="$perms.hasPermission('key500')">
              <template slot="title">
                <span>
                  <img class="title-img" src="../assets/img/fx.png" alt="">
                </span>
              ...
              <router-link :to="{path:'/ScenicArea'}" v-if="$perms.hasPermission('key500101')">
                <MenuItem name="/ScenicArea" class="f-fs4">景区管理</MenuItem>
              </router-link>
              <router-link :to="{path:'/KeyPeople'}" v-if="$perms.hasPermission('key500102')">
                <MenuItem name="/KeyPeople" class="f-fs4">重点人管理</MenuItem>
              </router-link>
             <router-link :to="{path:'/Management',query:{userId:this.userId}}" v-if="$perms.hasPermission('key500103')">
  ...
  export default {
    ...
    methods : {
      ...
      formatDate(date){
        ...
        if(this.$route.name=="population"||this.$route.name=="trajectory"){
          return y+'-'+this.timeFormatter(m)+'-'+this.timeFormatter(d);
        }else if(this.$route.name=="tour"||this.$route.name=="traffic"){
             mm=parseInt(mm/5)*5
