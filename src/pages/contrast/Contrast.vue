          ...
          <div class="remark">
            <p>
              ...
              <span v-show="!showRecommend">
                ...
                <span>区域：</span><span v-html="cityName"></span>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      export(path){
        window.location.href = urlApi.URL.exportExcel+"?path="+path;
      },
      //热力图导出
      exportHotMap(){
      ...
      sendHotMapData(params){
        if(params.id){
          window.location.href = urlApi.URL.heatMapExportExcel + "?key="+params.key+"&id="+params.id;
        }else {
          window.location.href = urlApi.URL.heatMapExportExcel + "?date="+params.date+"&cityOrDev="+params.cityOrDev+"&cityCode="+params.cityCode+"&dayOrNight="+params.dayOrNight+"&peopleType="+params.peopleType+"&originOrBelong="+params.originOrBelong+"&originOrBelongCode="+params.originOrBelongCode+"&pid="+params.pid+"&key="+params.key;
        }
      },

     //高级检索下热力图数据导出调用
      advanceHotMapData(params){
        window.location.href = urlApi.URL.heatMapExportExcel + "?date="+params.date+"&cityOrDev="+params.cityOrDev+"&contrastType="+params.contrastType+"&cityCode="+params.cityCode+"&dayOrNight="+params.dayOrNight+"&peopleType="+params.peopleType+"&originOrBelong="+params.originOrBelong+"&originOrBelongCode="+params.originOrBelongCode+"&pid="+params.pid+"&key="+params.key;
      },
      //热力图导出取消
      cancelHotType(){
     ...
     //base64转blob
      ...
      exportMyEchartsData(id){
        ...
        var len = myChart.getOption().series.length;
        let ylist = [];
        myChart.getOption().series.forEach((item,index)=>{
          if(item.data.length > 0){
            ylist.push(item.data.join(";"));
          }
        });
      ...
      queryNumChange(){
        this.$http.request({
          ...
          success:(data) => {
            ...
            this.initRelationActiveLine("#num-chart","人口统计折线图",legend,"人数/万人",data.data.xlist,data.data,true);
          },
          error : (data) => {
          }
        });
      ...
      queryGenRetrievalStatistics(params,num){
        this.$http.request({
          ...
          success:(data) => {
            this.initRelationActiveLine("#num-chart","人口统计折线图",[this.$refs['popType'+num].selectItem[0].name],"人数/万人",data.data.xlist,data.data,false);
          },
          error : (data) => {
          }
        });
        ...
        this.$http.request({
          ...
          success:(data) => {
            ...
            if(params.contrastType=="0"){
              this.initRelationActiveLineTime("#num-chart","人口统计折线图",legendData,"人数/万人",xListDatas,yListDatas);
            }else{
              this.initRelationActiveLineOther("#num-chart","人口统计折线图",legendData,"人数/万人",[xListDatas[0]],yListDatas)
            }
      ...
      sendUrl(){
        ...
        let remarkPar = me.showRecommend ? me.recommendRemark :
          ("时间："+me.remark.date + " 区域："+me.cityName + " 划分标准："+me.remark.dayOrNight
      ...
      checkSubmit(num){
        ...
        /*限制来源归属地省市对比*/
        if(num=='4'){
          if(this.$refs['popSource'+num].showSource=='0'&&this.$refs['popSource'+num].sourceValue.length!=0){
            let pro = this.$refs['popSource'+num].sourceValue.filter((item,index)=>{
              return  item.substring(2)=="0000";
            })
            let city =this.$refs['popSource'+num].sourceValue.filter((item,index)=>{
              return  item.substring(2)!="0000";
            })
            let flag = true;
            if(pro.length<this.$refs['popSource'+num].sourceValue.length) {
              pro.forEach((item) => {
                flag = city.some((x) => {
                  return x.substring(0, 2) == item.substring(0, 2)
                })
                if(!flag){
                  return;
                }
            ...
            if(!flag){
              this.$Message.warning("只能对比省或对比市");
              return;
            }
          }
          if(this.$refs['popSource'+num].showSource=='1'&&this.$refs['popSource'+num].belongValue.length!=0) {
            let pro = this.$refs['popSource' + num].belongValue.filter((item, index) => {
              return item.substring(2) == "0000";
            })
            let city = this.$refs['popSource' + num].belongValue.filter((item, index) => {
              return item.substring(2) != "0000";
            })
            let flag = true;
            if (pro.length < this.$refs['popSource' + num].belongValue.length) {
              pro.forEach((item) => {
                flag = city.some((x) => {
                  return x.substring(0, 2) == item.substring(0, 2)
                })
        ...
        let isProvice =false; //来源地/归属地是否是省份查询  true省级   false市级
        let sourceValue =this.$refs['popSource'+num].sourceValue;
        let belongValue =this.$refs['popSource'+num].belongValue;
        if(this.$refs['popSource'+num].sourceValue.length==1){
            isProvice =true;
        }else if(this.$refs['popSource'+num].sourceValue.length>1){
          if(Number(num)<4){
            isProvice =false;
            sourceValue=this.$refs['popSource'+num].sourceValue.filter((item,index)=>{
              return  item.substring(2)!="0000";
            })
          }else if(Number(num)==4){
            let flag =true; //都是省
            this.$refs['popSource'+num].sourceValue.forEach((item)=>{
        ...
        if(this.$refs['popSource'+num].belongValue.length==1){
          isProvice =true;
        }else if(this.$refs['popSource'+num].belongValue.length>1){


          if(Number(num)<4){
            isProvice =false;
            belongValue=this.$refs['popSource'+num].belongValue.filter((item,index)=>{
              return  item.substring(2)!="0000";
            })
          }else if(Number(num)==4){
            let flag =true; //都是省
            this.$refs['popSource'+num].belongValue.forEach((item)=>{
              if(item.substring(2)=="0000"){
                flag =true;
              }else{
                flag =false;
        ...
        this.cityName = this.$refs['area'+num].selectAreaName.join(",");
        this.remark.dayOrNight = this.$refs['area'+num].day;
        let peopleTypeName =[];
        if(this.$refs['popType'+num].selectItem.length!=0){
          this.$refs['popType'+num].selectItem.forEach((item)=>{
            peopleTypeName.push(item.name)
          })
        }
        this.remark.peopleType = peopleTypeName.join(",");
        ...
        switch(num){
          ...
          case '0':
            ...
            if(this.$route.query.type=='pop'){
              let params={
                ...
                originOrBelongCode:this.$refs['popSource'+num].showSource=='1' ? belongValue.join(',') :sourceValue.join(','),
                pid:"",
                provice:isProvice
              }
              ...
              let params={
                ...
                obList:this.$refs['popSource'+num].showSource=='1' ? belongValue :sourceValue,
                timeType:"0",//时间类型 0:分钟，1:日，2:月
                isPoint:this.$refs['time'+num].shortPoint ? '1':'0',//1时间点，0时间段；
                peopleFrom:(belongValue.length==0&&sourceValue.length==0) ? "0":"1" ,//人口来源模块 没值0有值1
                type:"5",
                provice:isProvice
              }
              let peopleIds =[]
          ...
          //高级检索-对比时间
          case '1':
            if(this.$refs['time'+num].timeVal2==""){
              this.$Message.warning("时间应选择应为两段时间，请完善选择条件！");
              return;
            }
            if(this.$refs['time'+num].timeVal2.length!=0){
              if(typeof(this.$refs['time'+num].timeVal2)=='string'){
                this.remark.date+=this.$refs['time'+num].timeVal2
              }
              if(typeof(this.$refs['time'+num].timeVal2)=='object'){
                this.remark.date+=this.$refs['time'+num].timeVal2.join("-")
              }
            };
            this.loadMap =false;
            this.isAdvance=true;
          ...
          //高级检索-对比区域
          case '2':
            if(this.$refs['area'+num].areaData.length<=1){
              this.$Message.warning("请至少选择两个区域！");
              return;
            }
            this.loadMap =false;
            this.isAdvance=true;
          ...
          //高级检索-对比人口类别
          case '3':
           if(this.$refs['popType'+num].selectItem.length<=1){
            this.$Message.warning("至少选择两个人口类别！");
            return;
          }
            this.loadMap =false;
            this.isAdvance=true;
          ...
          //高级检索-对比来源类型
          case '4':
            if(belongValue.length==0&&sourceValue.length==0){
              this.$Message.warning("请选择人口来源并具体到对应市！");
              return;
            }
            this.loadMap =false;
            this.isAdvance=true;
