<template>
    <div id="population-statistics">
      ...
      <div id="left-section" style="z-index: 999">
        ...
        <!--行为特征-->
        <div class="pop-list3">
          <Behaviour :dateData="dateData" :areaCode="areaCode" :areaName="areaName" :isCity="isCity" ref="behaviour"></Behaviour>
        </div>
      </div>
...
<script>
    ...
    export default {
      ...
      watch:{
        dateData(){
          // this.isCity = this.$refs.PopulationMapTool.isCity;
            ...
            success:(data) => {
              if(!!data.data){
                if(this.$area_Name.length>0){
                  this.areaName = this.$area_Name[0];
                  let flag = true;
                  this.data1=_.filter(data.data.list, (node)=> {
                    if(node.cityName==this.$area_Name[0]){
                      flag = false;
                      return [node]
                    }
                  })
