  ...
  <div class="resource-group">
    ...
    <CheckboxGroup v-model="checkedData" @on-change="handleCheckboxGroupChange">
      <div class="resource-group-item" v-for="item in stateData" :key="item.id">
        <div>
          <Checkbox :label="item.id" :disabled="!isTypeOfCheck">{{item.title}}</Checkbox>
        </div>
        <div v-if="item.children" class="resource-group-item-body">
          <Checkbox v-for="child in item.children" :key="child.id" :label="child.id" :disabled="!isTypeOfCheck">
            {{child.title}}
          </Checkbox>
        </div>
      </div>
     ...
     <span class="cls-titles">默认角色</span>
          <span>
                <i-Switch v-model=ischeck   @on-change="changeControl($event)" size="small" :disabled="!isTypeOfCheck"></i-Switch>
          </span>
  </div>
</template>
