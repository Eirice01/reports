<template>
    <div id="tour-modal">
      <Header v-if="realTitleTime.length!=0" :showMinute="showMinute" :realTitleTime="realTitleTime" @changeDateData="changeDateData" @showDistributeData="showDistributeData"
               @getTypeStatus=getTypeStatus ref="headers" @getOutRealtype="getOutRealtype"
               @getChangeTimeOutReal="getChangeTimeOutReal" @getChoseDayOrMourh="getChoseDayOrMourh"
               @handleChangeTep="handleChangeTep"@getchoseDayTime="getchoseDayTime"@gethandleMinute="gethandleMinute"
               @getPopStatus="getPopStatus"@getSourcetype="getSourcetype"@getOutStatus="getOutStatus"
               @tourChangeData="tourChangeData"@sourceNumChange="sourceNumChange"@changeTourtype="changeTourtype"@typeOfRealtime="typeOfRealtime"
      ...
      <div id="right-section" style="z-index: 2">
        <Row type="flex" style="height:100%">
          ...
          <div class="full-height right-list">
            <div class="list-table" :class="showDistribute ? 'full-table':'half-table'">
            ...
            <div class="pop-num">
              <!--人口变化统计（常驻，路过，游客）-->
              <PopNum  v-if="!showDistribute" :isType="isType" :dateData="dateData" ref="popNum" :scenicCode="scenicCode" :isclick="isclick"></PopNum>
            </div>
          </div>
        </Row>
      </div>
...
<script>
  ...
  import PopNum from "../../components/tour/PopNum"
    export default {
      name: "tour",
      data(){
        return{
      ...
      beforeDestroy(){
        ...
        clearInterval(this.$refs.popNum.numTimer);
        this.timers=null;
        this.aretimer=null;
      ...
      methods:{
        ...
        //旅游页面下流入流出非实时日，月，分切换
        tourChangeData(type,time){
         // this.$refs.populationnum.tourChangeDataType(type,time)
        },

        //旅游页面下非西安饼状图非实时日月分切换
        sourceNumChange(time,type){
          // this.$refs.populationform.sourceChangeType(time,type)
        },
        ...
        gethandleMinute(type,time){
          this.isclick=true;
           setTimeout(()=>{
             this.$refs.numchange.handleMinute(type,time);
             this.$refs.popNum.handleMinute(type,time);
           },50)
        ...
        getOutRealtype(status,type,time){
          setTimeout(()=>{
            ...
            this.$refs.popNum.getOutRealTime(status,type,time);
          },30)
        },
        //非实时下分钟选择后回调
        getChangeTimeOutReal(status,time){
            this.$refs.numchange.changeOutRealTime(status,time);
            this.$refs.popNum.changeOutRealTime(status,time);
        },
        //非实时下日，月切换回调
        ...
        //非实时下日，月选择时间后调用
        getchoseDayTime(times){
            setTimeout(()=>{
              // this.$refs.durationdistribute.choseDayOrMounth(times)
            },30)
        },
        //非实时下分，日,月 切换 调用
        ...
        typeOfRealtime(type){
          ...
          setTimeout(()=>{
            this.$refs.numchange.outRelaTimesData(parms);
            this.$refs.popNum.outRelaTimesData(parms);
          },40)
        ...
        areRealPagerData(){
          let flag='';
          if(this.scenicCode.includes("sub")){
            flag=1;
          }else if(this.scenicCode.includes("main")){
            flag=0;
          }
          this.marks=flag;
          let params={
        ...
        getSenicCode(code,name){
          ...
          this.$nextTick(()=>{
            if(this.dateData!=''){
              ...
              let popDom =document.getElementById("change-pop");//人口变化模块
              ...
              if(resNum){
                ...
                this.$refs.popNum.areinuteTimesData();
              }
...
<style scoped>
  ...
  #right-section .right-list{
    width: calc(100% - 25px);
  }
  #right-section .full-table{
    height: 100%;
  }
  #right-section .half-table{
    height: 60%;
  }
